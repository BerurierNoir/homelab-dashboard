...
class SettingsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          // ── Réseau ─────────────────────────────────────────────────
          _buildNetworkToggle(context, ref, servicesState.networkMode),
  ...
  Widget _buildNetworkToggle(
      BuildContext context, WidgetRef ref, NetworkMode mode) {
    final isLocal = mode == NetworkMode.local;
    return Container(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                _networkChip(context, ref, 'Local', NetworkMode.local, mode),
                _networkChip(
                    context, ref, 'Tailscale', NetworkMode.tailscale, mode),
              ],
            ),
          ),
  ...
  Widget _networkChip(BuildContext context, WidgetRef ref, String label,
      NetworkMode chipMode, NetworkMode current) {
    final sel = chipMode == current;
    final color = chipMode == NetworkMode.local
        ? const Color(0xFF1976D2)
        : const Color(0xFF7B1FA2);
  ...
  Future<void> _switchMode(
      BuildContext context, WidgetRef ref, NetworkMode mode) async {
    if (ref.read(servicesProvider).networkMode == mode) return;
    await ref.read(servicesProvider.notifier).setNetworkMode(mode);
    if (!context.mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(mode == NetworkMode.local
            ? 'Mode réseau local'
            : 'Mode Tailscale'),
        backgroundColor: mode == NetworkMode.local
